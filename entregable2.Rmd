...
- Quina relació hi ha entre la longitut del sèpal i la seva amplada? L'amplada influeix?
...
Si existís aquesta interacció, aleshores tindríem un coeficient d'interacció per a cada espècie (excepte per a l'espècie de referència), que s'interpretaria com a la diferència del que augmenta la longitut del sèpal per a cada unitat d'amplada entre una espècie amb l'espècie de referència.
...
Podem veure que l'hipòtesis de normalitat de residus no s'acaba de complir, ja que a la gràfica de Residuals vs Fitted els errors no son del tot homogenis (son més grans al voltant del valor 6), i a l'extrem del qq-plot hi ha una mica de corvatura.
...
Dues que potser ho son més són les mostres 118 i 132
...
Si, potser convindria treure aquelles observacions que són més influents en el model, ja que por ser que siguin algún error en la presa de dades o que no formin part del mateix conjunt.
Per tant, eliminarem aquelles mostres les quals tinguin un "leverage" superior a $3·p/n$.
...
Hem obtingut una variància residual de 0.4397, que és quasi la mateixa que teníem amb el model 2.

### 7.3) Interpreteu els paràmetres que us surtin significatius (una frase per a cada paràmetre).

Els paràmetres significatius que hem obtingut ja els haviem interpretat al model 2:
...
Potser una mica a la gràfica de Residual vs Fitted, però el qq-plot segueix sent molt dolent.
Això
...
- Les variables Petal.Width i Sepal.Length tenen bastanta correlació, cosa que voldriem evitar entre dues variables explicatives. Potser ens convindria agafar només una d'aquestes dues.
...
Només tenim una observació amb leverage; la 42.
...
Com que l'altra variable que té un VIF gran és la de Sepal.Length, sembla que l'amplada i la longitut del pètal estan correlades, tal com ja podiem veure al ggpairs de l'apartat 1).
...
Per tant, sí que podem afirmar que, estadísticament i amb un nivell de confiança del 95%, el coeficient de la variable amplada del pètal
...
```
Obtenim que el millor model és el model 5.
...
```{r}
plot(model5, which = 1)  # (o bé model4)
```

No ens cau cap residu fora de l'intèrval (-2, 2), quan la proporció de residus que cauen fora d'aquest intèrval hauria de representar un 5% aproximadament del total. Per tant no, no és un percentatge gaire escaient, ja que n'hauríem de tenir alguns, però al tenir poques dades tampoc podem rebutjar l'hipòtesi de normalitat de residus.
